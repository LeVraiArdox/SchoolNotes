...
<p className='text-red'>Ce cours est TREEEEEES long</p>


# Etude du reseau avec OSI et TCP/IP
...
<br />
<Diagram src="/reseaux/physique.png" />
<br />
...
<br />
<Diagram src="/reseaux/physique2.png" />
<br />
<div className="separator" />
## CSMA/CD et CSMA/CA
...
<br />
<Diagram src="/reseaux/csmacd.png" />
<br />
...
<br />
<Diagram src="/reseaux/csmaca.png" />
<br />
<div className="separator" />
...
- **Unicast** : il s’agit d’un mode de communication où il y a un expéditeur et un
destinataire, dont le rôle peut permuter. Comme dans une discussion.
- **Multicast : ou multi-diffusions** il s’agit d’un mode de communication ou un
expéditeur communique avec plusieurs destinataires. Comme par exemple dans
l’envoi d’un mail groupé
- **Broadcast** : il s’agit d’un mode de communication ou expéditeur communique avec
l’ensemble des membres d’un même réseau. Les destinataire peuvent ou non être
concernés par cette communication. Tous les appareils pouvant recevoir cette
émission font partie d'un domaine de diffusion. On peut comparer un broadcast avec
une émission de radio, de télé ou encore un stream sur Twitch mais cependant le
broadcast implique forcément que le destinataire soit connecté au réseau. Hors dans
les exemples cités, les utilisateurs ne sont pas automatiquement connectés à ces
réseaux.
- **Anycast** : il s’agit d’un mode de communication ou un expéditeur envoie un message
à un groupe de destinataires dont il est lui-même membre. Ici on se fiche de qui
réceptionne le message tant que celui-ci est réceptionné par un membre du groupe.
Cette méthode de communication est notamment utilisée pour le routage.
<br />
**Adresse MAC**
<br />
L'adresse Mac est un identifiant unique et propre à chaque carte réseau des équipements réseau (switch, routeur ...) ou
machines (ordinateur, serveur ...), au niveau du modèle OSI elle se situe à la couche 2 ; elle est codée sur 48 bits sur 6
octets : soit 48 bits et codé en hexadécimal (base16 : les chiffres de 0 à 9 plus les lettres de A à F majuscule ou
minuscule, que nous détaillerons dans le point suivant).
<br />
...
<br />
*Broadcast : Un broadcast dans un réseau d’ordinateurs est un message transmis à tous les participants du réseau et ne
nécessitant pas de réponse. L’un des ordinateurs du réseau envoie un paquet de données simultanément à tous les autres
participants du réseau. L’émetteur n’a pas à indiquer ici les adresses des destinataires, ce qui distingue le mode d’envoi
en broadcast du mode d’envoi en unicast. Dans l’envoi en unicast, l’expéditeur adresse la transmission à un seul
destinataire connu. De manière générale, l’avantage du broadcast est qu’il permet de diffuser très largement les
informations avec une seule et même transmission.
<br />
...
- Le binaire.
  ...
  La seule différence entre le calcul binaire et décimal, réside dans le fait que la base 2 ne nous donne que deux caractères
  pour représenter un digit. Donc la retenue qui intervient toutes les 10 caractères en décimal interviendra tous les 2
  caractères en binaire.
  Comme pour la base 10 où chaque nombre peut être exprimé en puissance. En binaire chaque bit peut être exprimé par
  une puissance de 2 puisque nous sommes en base 2. Cela signifie que chaque digit sera deux fois plus grand que le
  précédent.
  <Diagram src='/reseaux/bintable.png' />
  Comment faire conversion décimale vers binaire ?
  Pour effectuer la conversion du décimal vers le binaire il faut fait appel à la fonction ET. Pour rappel la fonction ET est un
  comparateur logique en algèbre qui permet de vérifier si deux valeurs respectent toutes les deux une condition, le
  résultat de cette comparaison peut être VRAI ou FAUX. Si le résultat est VRAI (true en anglais) le bit sera à 1, si c’est
  FAUX (false en anglais) le bit sera à 0.
  Pour effectuer la conversion il faut vérifier si la valeur décimale est ≥ à la valeur de la puissance de 2. Si cette
  comparaison est vraie, le bit correspondant est à 1, dans le cas contraire le bit est à 0.
  <Diagram src='/reseaux/bintodec.png' />
  Dans notre cas 21 s’écrit en binaire : 10101. Nous ne sommes pas obligés dans une simple conversion d’écrire les
  premiers bits à 0, ils représentent l’absence de valeur ils sont donc le cas présent inutiles.
  On peut donc maintenant également expliquer les fameuses 256 valeurs possibles dans un octet que nous avons évoqué
  en parlant de l’adresse mac . Si on multiplie le nombre de valeurs possibles par bit (2) par celle sdes autres bits on
  obtient le calcul suivant : 2x2x2x2x2x2x2x2 = 256.
  Maintenant que nous avons fait une première conversion binaire, je vais vous posez la question suivante pourquoi les
  puissances de 2 sont rangées dans l’ordre décroissant dans un octet et pas dans l’ordre croissant ?
  La réponse vient du fait que si l’on ne range pas les puissances de deux dans l’ordre croissant on ne pourra pas convertir
  l’ensemble de nombre décimaux.
<br />
- L'hexadécimal.

  Il s’agit du système hexadécimal qui est tout aussi important en informatique que le binaire notamment dans le
  développement, mais aussi en réseau avec notamment les adresses MAC.
  En mathématique nous n’avons que 10 chiffres de 0 à 9, donc dix caractères possibles, et là nous devons coder avec 16
  caractères.
  Du coup la question qui se pose est : comment faire pour avoir 16 caractères?
  Et bien c’est très simple en hexadécimal nous avons ajouté en plus des chiffres de 0 à 9 les lettres de A à F (en majuscule
  ou minuscule), ce qui nous donne le tableau de valeur suivant :
  <Diagram src='/reseaux/hextable.png' />
  ...
  **Conversion binaire vers hexadécimal**
  ...
  <Diagram src='/reseaux/binhex.png' />
...
- On commence par les adresses MAC destinataires, suivies de la MAC de l’expéditeur codé sur 6 octets chacune
  comme vu précédemment. Les chercheurs ont déterminé qu’il était plus intéressant d’avoir l’adresse du destinataire
